...
(function () {
  function addCombo(editor, comboName, styleType, lang, entries, defaultLabel, styleDefinition, order) {
    ...
    editor.ui.addRichCombo(comboName, {
      ...
      onClick: function (value) {
        ...
        // When applying one style over another, first remove the previous one
        // (http://dev.ckeditor.com/ticket/12403). NOTE: This is only a
        // temporary fix. It will be moved to the styles system
        // (http://dev.ckeditor.com/ticket/12687).
        if (previousValue && value != previousValue) {
          var previousStyle = styles[previousValue],
              range = editor.getSelection().getRanges()[0];

          // If the range is collapsed we can't simply use the
          // editor.removeStyle method because it will remove the entire
          // element and we want to split it instead.
          if (range.collapsed) {
            ...
            if (matching) {
              ...
              // If we are at both boundaries it means that the element is
              // empty. Remove it but in a way that we won't lose other empty
              // inline elements inside it. Example: <p>x<span
              // style="font-size:48px"><em>[]</em></span>x</p> Result:
              // <p>x<em>[]</em>x</p>
              if (startBoundary && endBoundary) {
                bm = range.createBookmark();
                // Replace the element with its children (TODO
                ...
                // If we are at the boundary of the style element, move out and
                // copy nested styles/elements.
              }
                ...
                // Split the element and clone the elements that were in the
                // path (between the startContainer and the matching element)
                // into the new place.
  ...
  //
  // @param range
  // @param {CKEDITOR.dom.element[]} elements Elements path in the standard
  // order: leaf -> root. @param {CKEDITOR.dom.element/null} substreeStart The
  // start of the subtree. If null, then the leaf belongs to the subtree.
  function cloneSubtreeIntoRange(range, elements, subtreeStart) {
    var current = elements.pop();
    if (!current) {
      return;
    }
    // Rewind the elements array up to the subtreeStart and then start the real
    // cloning.
    if (subtreeStart) {
 ...
 * name and the slash character. For example, `'Arial/Arial, Helvetica,
 * sans-serif'` will be displayed as `'Arial'` in the list, but will be
 * outputted as
 ...
 *
 *    // If the default site font is Arial, we may making it more explicit to
 * the end user. config.font_defaultLabel = 'Arial';
 *
 ...
 *
 *    // If the default site font size is 12px, we may making it more explicit
 * to the end user. config.fontSize_defaultLabel = '12px';
 *
