...
if(isset($_POST['login']))
{
      ...
      if($email !="" && $UserName!="" && $Password!="" && $firstname!="" && $lastname!="" && $address!="" && $city!="" && $state!="" && $postal!="" && $phone!="")
    {
      ...
      $param=array('UserName' =>"$UserName",'Email' =>"$email",'PublicationID'=>"$publicationid");
      //$param=array('Email' =>"$email",'UserName' =>"$UserName",'Password' =>"$Password",'FirstName' =>"$firstname",'LastName'=>"$lastname",'StreetAddress'=>"$address",'City'=>"$city",'State'=>"$state",'PostalCode'=>"$postal",'Phone'=>"$phone",'ExpiryDate'=>"$expdate",'PublicationID'=>"$publicationid",'SubscriptionPlanID'=>"$planid");
         ...
         // $client3= new soapclient('http://etype.sublimeitsolutions.com/Service_SubsciberRegistration.asmx?wsdl');
       ...
       if($response3->CheckUserNameAndEmailResult == -1)
        ...
        else if($response3->CheckUserNameAndEmailResult == -2)
             {
               $msg="Email Already Registred aginst this publication";
              
             }
           else if($response3->CheckUserNameAndEmailResult == -3)
             {
       ...
       drupal_goto('node/56728');
               
   }
    }
 else
 {
$msg="Some Error On Registration. Please try Again";
 }
}
...
<form name="form2" id="form2" method="POST" action="" enctype="multipart/form-data">
        ...
        <table style="width:100%">
            <tr>
                <th style="border: 2px solid gray;background: #ccc"><strong>Quantity</strong></th>
                <th style="border: 2px solid gray;background: #ccc"><strong>Product</strong></th>
                <th style="border: 2px solid gray;background: #ccc"><strong>Price</strong></th>
            </tr>
            <tr>
                 <td>1*</td>
                <td> <?php echo $response->GetSubsciptionPlanResult->PlanDetails[$i]->Title;   echo $response->GetSubsciptionPlanResult->PlanDetails[$i]->Period; echo $response->GetSubsciptionPlanResult->PlanDetails[$i]->PeriodType; ?></td>
                <td>$ <?php echo $response->GetSubsciptionPlanResult->PlanDetails[$i]->Price ;?></td>
                <input type="hidden" name="Publication_Id" value="<?php echo $_POST['Publication_Id'] ?>">
                <input type="hidden" name="planId" value="<?php echo $response->GetSubsciptionPlanResult->PlanDetails[$i]->ID; ?>">
                <input type="hidden" name="Period" value="<?php echo $response->GetSubsciptionPlanResult->PlanDetails[$i]->Period; ?>">
                <input type="hidden" name="PeriodType" value="<?php echo $response->GetSubsciptionPlanResult->PlanDetails[$i]->PeriodType; ?>">
                <input type="hidden" name="planTitle" value="<?php echo $response->GetSubsciptionPlanResult->PlanDetails[$i]->Title; ?>">
  ...
  <?php  $i++; } else {    $i++;      } } ?>
        ...
        <div style="margin-left:30px">        
           ...
           <p> If you are signing up for a 3-month, 6-month or 1-year subscription, this is where we will deliver the latest e-Edition of the newspaper.</p>
 
            <div>
                <label for="name">UserName:</label>
                <input type="text" name="name" id="username" class="txt" required="required"/>
            ...
            <div>
             ...
             <div>
                <label for="cpassword">Confirm Password:</label>
                 ...
                 <input type="password" name="cpassword"  id="cpassword" pattern=".{6,}"  placeholder="Confirm Password" oninput="check(this)" class="txt" required="required" />
            </div></div><br/>
            <br/>
...
Enter your billing address and information here.</p>
 ...
 <br/>
            ...
            <div style="width: 93px;float: right;background-color: gainsboro;height: 26px;border: 1px solid #ccc;border-radius: 4px;text-shadow: white 0 1px 1px;"><p style="margin-left: 30px;margin-top: 4px;">Back</p></div></a>
            
            

              <!--  <input type="submit" name="login" value="Review Order!" /> -->
        <input type="submit" name="login" value="Review Order!" style="float:left;background-color: gainsboro;border-radius: 4px;width: 96px;height: 28px;border: 1px solid #CCC;text-decoration: none;color: #000;text-shadow: white 0 1px 1px;padding: 2px;"></div>
